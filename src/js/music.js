...
$.ajax({
    ...
    success: function fn(data) {
        ...
        var str='';
        res.forEach(function(item, index){
            str+='<div class="music clearfix"><div class="fl"><p class="song">'+item.title+'</p><p class="author">'+item.author
            +'</p></div><div class="fr"><i class="iconfont icon-playon"></i></div></div>'
        }),
        $(".new").html(str);
    },
    error: function error(err) {
...
$.ajax({
    ...
    success: function ag(data) {
        ...
        var str='';
        res.forEach(function(item, index){
            str+='<a class="floor1"><div class="music clearfix"><div class="num fl">'+(index+1)
            +'</div><div class="fl"><p class="song">'+item.title+'</p><p class="author">'+item.author
            +'</p></div><div class="fr"><i class="iconfont icon-playon"></i></div></div></a>'
        }),
        $(".agog").html(str);
    },
    error: function error(err) {
    ...
    li[i].onclick=function(){
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                res.forEach(function(item, index){
                    console.log("1");
                    str+='<a class="floor1"><div class="music clearfix"><div class="num fl"><p class="song">'+item.songname+'</p><p class="author">'+item.artistname
                    +'</p></div><div class="fr"><i class="iconfont icon-playon"></i></div></div></a>'
                }),
                $(".div5").html(str);
            },
...
for(let i=0;i<m_btn.length;i++){
    m_btn[i].onclick=function(){
        for(let j=0;j<m_btn.length;j++){
            m_btn[j].className="";
            m_content[j].className="";
        }
        this.className="active";
        m_content[i].className="active";
    }
}
/*---获取cookie---*/
